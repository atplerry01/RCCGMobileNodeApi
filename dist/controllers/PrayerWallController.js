"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var class_validator_1 = require("class-validator");
var PrayerWall_1 = require("../entity/PrayerWall");
var PrayerWall_2 = require("../services/PrayerWall");
var pagination_1 = require("../utils/pagination");
var PrayerWallController = /** @class */ (function () {
    function PrayerWallController() {
    }
    PrayerWallController.all = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var _a, page, per_page, entity, result, error_1;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _a = req.query, page = _a.page, per_page = _a.per_page;
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, PrayerWall_2.getPrayerWallService()];
                case 2:
                    entity = _b.sent();
                    return [4 /*yield*/, pagination_1.Paginator(entity, page, per_page)];
                case 3:
                    result = _b.sent();
                    return [2 /*return*/, res.status(200).json({
                            success: true,
                            data: result,
                        })];
                case 4:
                    error_1 = _b.sent();
                    return [2 /*return*/, false];
                case 5: return [2 /*return*/];
            }
        });
    }); };
    PrayerWallController.getOneById = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, name, entity, error_2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    id = req.params.id;
                    name = req.body.name;
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, PrayerWall_2.getPrayerWallByIdService(id)];
                case 2:
                    entity = _a.sent();
                    if (entity.success) {
                        return [2 /*return*/, res.status(200).json({
                                success: entity.success,
                                data: entity.data,
                            })];
                    }
                    else {
                        return [2 /*return*/, res.status(400).json({
                                success: entity.success,
                                msg: entity.msg,
                            })];
                    }
                    return [3 /*break*/, 4];
                case 3:
                    error_2 = _a.sent();
                    return [2 /*return*/, res.status(400).json({
                            success: false,
                            msg: error_2,
                        })];
                case 4: return [2 /*return*/];
            }
        });
    }); };
    PrayerWallController.create = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var _a, title, summary, details, parishName, prayerWall, errors, error_3;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    _a = req.body, title = _a.title, summary = _a.summary, details = _a.details, parishName = _a.parishName;
                    prayerWall = new PrayerWall_1.PrayerWall();
                    prayerWall.title = title;
                    prayerWall.summary = summary;
                    prayerWall.details = details;
                    prayerWall.parishName = parishName;
                    return [4 /*yield*/, class_validator_1.validate(prayerWall)];
                case 1:
                    errors = _b.sent();
                    if (errors.length > 0) {
                        res.status(400).send({
                            success: false,
                            msg: errors,
                        });
                        return [2 /*return*/];
                    }
                    _b.label = 2;
                case 2:
                    _b.trys.push([2, 4, , 5]);
                    return [4 /*yield*/, PrayerWall_2.createPrayerWallService(prayerWall)];
                case 3:
                    _b.sent();
                    return [2 /*return*/, res.status(201).json({
                            success: true,
                        })];
                case 4:
                    error_3 = _b.sent();
                    res.status(400).send({
                        success: false,
                        msg: 'something went wrong',
                    });
                    return [2 /*return*/];
                case 5: return [2 /*return*/];
            }
        });
    }); };
    PrayerWallController.update = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, _a, title, summary, details, parishName, entity, prayerWall, errors, error_4;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    id = req.params.id;
                    _a = req.body, title = _a.title, summary = _a.summary, details = _a.details, parishName = _a.parishName;
                    _b.label = 1;
                case 1:
                    _b.trys.push([1, 5, , 6]);
                    return [4 /*yield*/, PrayerWall_2.getPrayerWallByIdService(id)];
                case 2:
                    entity = _b.sent();
                    if (!entity.success) {
                        return [2 /*return*/, res.status(400).json({
                                success: false,
                                msg: entity.msg,
                            })];
                    }
                    prayerWall = entity.data;
                    prayerWall.title = title;
                    prayerWall.summary = summary;
                    prayerWall.details = details;
                    prayerWall.parishName = parishName;
                    return [4 /*yield*/, class_validator_1.validate(prayerWall)];
                case 3:
                    errors = _b.sent();
                    if (errors.length > 0) {
                        res.status(400).send({
                            success: false,
                            msg: errors,
                        });
                        return [2 /*return*/];
                    }
                    return [4 /*yield*/, PrayerWall_2.updatePrayerWallService(prayerWall)];
                case 4:
                    _b.sent();
                    return [2 /*return*/, res.status(200).json({
                            success: true
                        })];
                case 5:
                    error_4 = _b.sent();
                    res.status(400).send({
                        success: false,
                        msg: 'something went wrong',
                    });
                    return [2 /*return*/];
                case 6: return [2 /*return*/];
            }
        });
    }); };
    PrayerWallController.delete = function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
        var id, entity, error_5;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    id = req.params.id;
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, PrayerWall_2.getPrayerWallByIdService(id)];
                case 2:
                    entity = _a.sent();
                    if (!entity.success) {
                        return [2 /*return*/, res.status(400).json({
                                success: false,
                                msg: entity.msg,
                            })];
                    }
                    return [4 /*yield*/, PrayerWall_2.deletePrayerWallService(id)];
                case 3:
                    _a.sent();
                    return [2 /*return*/, res.status(200).json({
                            success: true,
                        })];
                case 4:
                    error_5 = _a.sent();
                    res.status(400).send({
                        success: false,
                        msg: 'something went wrong',
                    });
                    return [2 /*return*/];
                case 5: return [2 /*return*/];
            }
        });
    }); };
    return PrayerWallController;
}());
exports.default = PrayerWallController;
